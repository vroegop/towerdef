/* huds/arcade.js — the "Arcade" CRT-cabinet skin (Set A) as a REAL, full-parity HUD.

   Like huds/dnd.js, this is NOT a fork of the HUD logic: it reuses the single themeable core in
   hud/hud.js via A.createThemedHud({ cls, css }). Same DOM, same handlers, same behaviour as
   Classic — only restyled into a high-contrast pixel cabinet.

   Palette + type lifted from the Set A prototype (huds/_prototype-hud-gallery.html, theme-arcade):
     bg #101018 / panel #1a1a2e · ink #fff · accent #ffd23f (yellow) · accent2 #06d6a0 (teal)
     crimson #d62828 · pink #ef476f · rule #3a3a5e · hard #000 drop-shadows · radius 0
     font: 'Press Start 2P'

   Eager-loaded. Self-contained apart from the Google Fonts @import in the injected stylesheet. */
(function (A) {
  const CSS = `
...
`;
  ...
  A.createArcadeHud = A.createThemedHud({ cls: 'theme-arcade', css: CSS });
