...
// Permanent COIN price (all four share it): 30 → ×1.01+1/level to 150, ×1.005/level to 1k, ×1.003
// to 5k, ×1.008 beyond (a sharp end-game ramp the player can't keep up with, but not runaway). In-run
// GOLD price keeps each skill's own start, then ramps gently: ×1.01+1/level to 100, ×1.005 to 1k,
// ×1.003 to 3k, ×1.002 to 6k.
const DEEP_COIN_SEGS: [number, number, number][] = [[150, 1.01, 1], [1000, 1.005, 0], [5000, 1.003, 0], [Infinity, 1.008, 0]];
const DEEP_GOLD_SEGS: [number, number, number][] = [[100, 1.01, 1], [1000, 1.005, 0], [3000, 1.003, 0], [Infinity, 1.002, 0]];
...
const UPGRADE_SPECS: UpgradeSpec[] = [
  ...
  { id: 'msChance', tab: 'attack', icon: 'bow', label: 'Multishot',
    ...
    tip: (up) => 'Chance each attack fires extra projectiles at nearby targets. Max: ' + up.fmt(up.value(up.max)) + '.',
    max: 99, curve: { kind: 'linear', base: 0, per: 0.005, cap: 0.495 }, fmt: pctFmt,
    gold: curve(100, 1.6), coin: curve(25, 1.0018) },
  { id: 'msTargets', tab: 'attack', icon: 'bow', label: 'Multishot',
    name: 'Multishot Targets',
    tip: (up) => 'Maximum extra targets hit per multishot. Max: ' + up.fmt(up.value(up.max)) + '.',
    max: 8, curve: { kind: 'linear', base: 1, per: 1 }, fmt: (v) => '' + v,
    gold: curve(250, 1.7), coin: curve(40, 1.02) },
  { id: 'bounceChance', tab: 'attack', icon: 'arrow', label: 'Bounce',
    name: 'Bounce Chance',
    tip: (up) => 'Chance a projectile bounces to another enemy after impact. Max: ' + up.fmt(up.value(up.max)) + '.',
    max: 85, curve: { kind: 'linear', base: 0, per: 0.008, cap: 0.68 }, fmt: pctFmt,
    gold: tcurve(BOUNCECHANCE_COST), coin: tcurve(BOUNCECHANCE_COST) },
  { id: 'bounceTargets', tab: 'attack', icon: 'arrow', label: 'Bounce',
    name: 'Bounce Targets',
    tip: (up) => 'Maximum enemies a bouncing projectile can chain through. Max: ' + up.fmt(up.value(up.max)) + '.',
  ...
  { id: 'bounceRange', tab: 'attack', icon: 'range', label: 'Bnc Range',
    name: 'Bounce Range',
    tip: (up) => 'Maximum distance for a bounce. Base: ' + up.fmt(up.value(0)) + ', max: ' + up.fmt(up.value(up.max)) + '.',
  ...
  { id: 'thorns', tab: 'defense', icon: 'shield', label: 'Reflect',
    name: 'Reflect',
    tip: 'Reflects a fraction of incoming damage back to the attacker.',
    max: 99, curve: { kind: 'linear', base: 0, per: 0.01, cap: 0.99 }, fmt: (v) => '+' + v.toFixed(2) + '×',
...
export const UNLOCK_COST_OVERRIDE: Record<string, number> = {
  ...
  msChance: 400, msTargets: 400,
  rapidChance: 1500, rapidDuration: 1500,
...
const RAW_GROUPS: { id: string; label: string; skills: string[] }[] = [
  ...
  { id: 'reflect', label: 'Reflect', skills: ['thorns'] },
...
const CARD_SPECS: Record<string, CardSpec> = {
  ...
  plasmaCanon: { id: 'plasmaCanon', name: 'Plasma Canon', art: 'burst', tint: '#37d7ff', rarity: 'rare',
    effects: [{ stat: 'plasmaCanon', kind: 'active' }], active: { cooldown: 12 },
    curve: tbl([[1, 0.30], [3, 0.34], [5, 0.38], [7, 0.42], [9, 0.46], [11, 0.50], [13, 0.54], [15, 0.58]]),
    fmt: (v) => '-' + (v * 100).toFixed(0) + '%', desc: (v) => 'boss hit -' + (v * 100).toFixed(0) + '% max HP' },
...
export const CARD_INFO: Record<string, string> = {
  ...
  plasmaCanon: 'Periodically blasts a boss for a share of its max HP.',
