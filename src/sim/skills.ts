...
export const UPGRADES: UpgradeDef[] = [
  ...
  { id: 'attackSpeed', tab: 'attack', icon: 'rate', label: 'Atk Speed',
    name: 'Attack Speed', tip: 'Shots fired per second. Scales from 1/s to 20/s over 100 levels.',
    max: 100, value: (b) => 1 + b * 0.19, fmt: (b) => (1 + b * 0.19).toFixed(1) + '/s',
    gold: curve(15, 1.6), coin: curve(3, 1.05) },
  { id: 'rangedDamage', tab: 'attack', icon: 'bow', label: 'Ranged DMG',
    name: 'Ranged Damage', tip: 'Flat damage added to every projectile hit.',
    max: 10000, value: (b) => 1 + b, fmt: (b) => '' + (1 + b), gold: curve(10, 1.5), coin: curve(4, 1.0018) },
  { id: 'dmgPerMeter', tab: 'attack', icon: 'ruler', label: 'DMG/m',
    name: 'Damage per Metre', tip: 'Each metre between the hero and the target multiplies damage. A target 50 m away with +0.1×/m doubles your damage.',
    max: 10000, value: (b) => b * 0.001, fmt: (b) => '+' + (b * 0.001).toFixed(3) + '×/m',
    gold: curve(25, 1.55), coin: curve(6, 1.0018) },
  { id: 'range', tab: 'attack', icon: 'range', label: 'Range',
    name: 'Attack Range', tip: 'Maximum distance at which the hero can target enemies. Starts at 50 m, caps at 150 m.',
    max: 100, value: (b) => BASE_RANGE_M + b,
    fmt: (b) => (BASE_RANGE_M + b) + 'm', gold: curve(1, 1.2), coin: curve(1, 1.2) },
  { id: 'critChance', tab: 'attack', icon: 'crit', label: 'Crit Chance',
    name: 'Critical Hit Chance', tip: 'Probability each shot is a critical hit, dealing increased damage.',
    max: 1200, value: (b) => Math.min(1.2, b * 0.001), fmt: (b) => (Math.min(1.2, b * 0.001) * 100).toFixed(1) + '%',
    gold: curve(40, 1.6), coin: curve(20, 1.008) },
  { id: 'critDamage', tab: 'attack', icon: 'burst', label: 'Crit DMG',
    name: 'Critical Damage', tip: 'Multiplier applied to damage on a critical hit.',
    max: 10000, value: (b) => 1 + b * (999 / 10000),
    fmt: (b) => { const v = 1 + b * (999 / 10000); return (v < 10 ? v.toFixed(1) : v.toFixed(0)) + '×'; },
    gold: curve(50, 1.6), coin: curve(25, 1.0018) },
  { id: 'superCrit', tab: 'attack', icon: 'burst', label: 'Super Crit',
    name: 'Super Critical Chance', tip: 'Chance that a critical hit deals a further ×10 damage.',
    max: 1000, value: (b) => Math.min(1, b * 0.001), fmt: (b) => (Math.min(1, b * 0.001) * 100).toFixed(1) + '%',
    gold: curve(80, 1.6), coin: curve(25, 1.0018) },
  { id: 'rendChance', tab: 'attack', icon: 'crit', label: 'Rend Chance',
    name: 'Rend Chance', tip: 'Probability each hit applies a Rend stack, boosting future damage to that enemy.',
    max: 1000, value: (b) => Math.min(1, b * 0.001), fmt: (b) => (Math.min(1, b * 0.001) * 100).toFixed(1) + '%',
    gold: curve(90, 1.6), coin: curve(25, 1.0018) },
  { id: 'rendMult', tab: 'attack', icon: 'burst', label: 'Rend Power',
    name: 'Rend Power', tip: 'Damage bonus per Rend stack on the target. Stacks decay after a few seconds.',
    max: 1000, value: (b) => b * 0.002, fmt: (b) => '+' + (b * 0.002 * 100).toFixed(1) + '%/stack',
    gold: curve(90, 1.6), coin: curve(25, 1.0018) },
  { id: 'msChance', tab: 'attack', icon: 'bow', label: 'Multishot',
    name: 'Multishot Chance', tip: 'Chance each shot fires additional projectiles at nearby targets.',
    max: 1000, value: (b) => Math.min(1, b * 0.001), fmt: (b) => (Math.min(1, b * 0.001) * 100).toFixed(1) + '%',
    gold: curve(100, 1.6), coin: curve(25, 1.0018) },
  { id: 'msTargets', tab: 'attack', icon: 'bow', label: 'Multi Tgts',
    name: 'Multishot Targets', tip: 'Maximum number of extra targets hit during a multishot.',
    max: 8, value: (b) => 1 + b, fmt: (b) => '' + (1 + b),
    gold: curve(250, 1.7), coin: curve(40, 1.02) },
  { id: 'bounceChance', tab: 'attack', icon: 'arrow', label: 'Bnc Shot',
    name: 'Bounce Shot Chance', tip: 'Chance a projectile bounces to a nearby enemy after hitting its target.',
    max: 1000, value: (b) => Math.min(1, b * 0.001), fmt: (b) => (Math.min(1, b * 0.001) * 100).toFixed(1) + '%',
    gold: curve(100, 1.6), coin: curve(25, 1.0018) },
  { id: 'bounceTargets', tab: 'attack', icon: 'arrow', label: 'Bnc Tgts',
    name: 'Bounce Targets', tip: 'Maximum number of enemies a bouncing projectile can hit.',
    max: 10, value: (b) => 1 + b, fmt: (b) => '' + (1 + b),
    gold: curve(250, 1.7), coin: curve(40, 1.02) },
  { id: 'bounceRange', tab: 'attack', icon: 'range', label: 'Bnc Range',
    name: 'Bounce Range', tip: 'Maximum distance a bouncing projectile can travel to its next target.',
    max: 10000, value: (b) => 120 + b, fmt: (b) => Math.round((120 + b) / PX_PER_METER) + 'm',
    gold: curve(60, 1.5), coin: curve(10, 1.0018) },
  { id: 'rapidChance', tab: 'attack', icon: 'rate', label: 'Rapid Fire',
    name: 'Rapid Fire Chance', tip: 'Chance every few seconds to trigger a burst of 3× attack speed.',
    max: 1000, value: (b) => Math.min(1, b * 0.001), fmt: (b) => (Math.min(1, b * 0.001) * 100).toFixed(1) + '%',
    gold: curve(100, 1.6), coin: curve(25, 1.0018) },
  { id: 'rapidDuration', tab: 'attack', icon: 'rate', label: 'Rapid Dur.',
    name: 'Rapid Fire Duration', tip: 'How long the Rapid Fire burst lasts when triggered.',
    max: 480, value: (b) => 2 + b * 0.1, fmt: (b) => (2 + b * 0.1).toFixed(1) + 's',
    gold: curve(120, 1.6), coin: curve(20, 1.0018) },

  // ---- DEFENSE ----
  { id: 'health', tab: 'defense', icon: 'heart', label: 'Health',
    name: 'Max Health', tip: 'Increases the hero\'s maximum HP.',
    max: 10000, value: (b) => 1 + b, fmt: (b) => '' + (1 + b), gold: curve(10, 1.5), coin: curve(4, 1.0018) },
  { id: 'regen', tab: 'defense', icon: 'regen', label: 'HP Regen',
    name: 'Health Regeneration', tip: 'HP recovered per second while out of combat.',
    max: 10000, value: (b) => b * 0.2, fmt: (b) => (b * 0.2).toFixed(1) + '/s',
    gold: curve(20, 1.6), coin: curve(5, 1.0018) },
  { id: 'dodge', tab: 'defense', icon: 'dodge', label: 'Dodge',
    name: 'Dodge Chance', tip: 'Probability of completely avoiding an incoming hit.',
    max: 1100, value: (b) => Math.min(0.99, b * 0.0009), fmt: (b) => (Math.min(0.99, b * 0.0009) * 100).toFixed(1) + '%',
    gold: curve(40, 1.6), coin: curve(20, 1.008) },
  { id: 'armor', tab: 'defense', icon: 'shield', label: 'Armor',
    name: 'Armor', tip: 'Flat damage reduction subtracted from every incoming hit.',
    max: 10000, value: (b) => b, fmt: (b) => '-' + b, gold: curve(30, 1.55), coin: curve(6, 1.0018) },
  { id: 'defPct', tab: 'defense', icon: 'shield', label: 'Defense %',
    name: 'Defense Percentage', tip: 'Percentage damage reduction applied after Armor.',
    max: 900, value: (b) => Math.min(0.9, b * 0.001), fmt: (b) => (Math.min(0.9, b * 0.001) * 100).toFixed(1) + '%',
    gold: curve(50, 1.6), coin: curve(20, 1.008) },
  { id: 'thorns', tab: 'defense', icon: 'shield', label: 'Thorns',
    name: 'Thorns', tip: 'Reflects a fraction of melee damage back to the attacker.',
    max: 10000, value: (b) => b * 0.05, fmt: (b) => '+' + (b * 0.05).toFixed(2) + '×',
    gold: curve(35, 1.55), coin: curve(8, 1.0018) },
  { id: 'lifesteal', tab: 'defense', icon: 'regen', label: 'Lifesteal',
    name: 'Lifesteal', tip: 'Heals the hero for a percentage of all damage dealt.',
    max: 500, value: (b) => Math.min(0.25, b * 0.0005), fmt: (b) => (Math.min(0.25, b * 0.0005) * 100).toFixed(1) + '%',
    gold: curve(60, 1.6), coin: curve(15, 1.0018) },

  // ---- ECONOMIC (Tier 2+) ----
  { id: 'goldPerWave', tab: 'economic', icon: 'coin', label: 'Gold/Wave',
    name: 'Gold per Wave', tip: 'Bonus gold awarded at the start of each wave.',
    max: 10000, gated: true, value: (b) => b, fmt: (b) => '+' + b, gold: curve(30, 1.55), coin: curve(8, 1.0018) },
  { id: 'goldPerKill', tab: 'economic', icon: 'coin', label: 'Gold/Kill',
    name: 'Gold per Kill', tip: 'Multiplier on gold earned from killing enemies.',
    max: 10000, gated: true, value: (b) => b * 0.1, fmt: (b) => '+' + (b * 0.1).toFixed(1) + '×',
    gold: curve(50, 1.6), coin: curve(12, 1.0018) },
  { id: 'cashBonus', tab: 'economic', icon: 'coin', label: 'Cash Bonus',
    name: 'Cash Bonus', tip: 'Global multiplier applied to all gold earned.',
    max: 10000, gated: true, value: (b) => 1 + b * 0.02, fmt: (b) => '×' + (1 + b * 0.02).toFixed(2),
    gold: curve(40, 1.55), coin: curve(10, 1.0018) },
  { id: 'interest', tab: 'economic', icon: 'coin', label: 'Interest',
    name: 'Interest', tip: 'Percentage of your current gold earned as a bonus each wave.',
    max: 500, gated: true, value: (b) => b * 0.002, fmt: (b) => (b * 0.002 * 100).toFixed(1) + '%/wave',
    gold: curve(80, 1.6), coin: curve(20, 1.0018) },
  { id: 'maxInterest', tab: 'economic', icon: 'coin', label: 'Max Interest',
    name: 'Interest Cap', tip: 'Upper limit on gold that can be earned from interest per wave.',
    max: 10000, gated: true, value: (b) => 50 + b * 10, fmt: (b) => '≤' + (50 + b * 10),
    gold: curve(40, 1.55), coin: curve(10, 1.0018) },
  { id: 'freeUp', tab: 'economic', icon: 'coins', label: 'Free Ups.',
    name: 'Free Upgrades', tip: 'Chance each in-run upgrade purchase costs no gold.',
    max: 200, gated: true, value: (b) => Math.min(0.5, b * 0.0025), fmt: (b) => (Math.min(0.5, b * 0.0025) * 100).toFixed(1) + '%',
    gold: curve(120, 1.6), coin: curve(25, 1.0018) },
  { id: 'waveCut', tab: 'economic', icon: 'rate', label: 'Wave Speed',
    name: 'Wave Speed', tip: 'Reduces the countdown between waves.',
    max: 5, gated: true, value: (b) => Math.min(5, b), fmt: (b) => '-' + Math.min(5, b) + 's',
    gold: curve(5000, 4), coin: curve(2000, 3) },
  { id: 'coinsPerWave', tab: 'economic', icon: 'coinstar', label: 'Coins/Wave',
    name: 'Coins per Wave', tip: 'Bonus coins (permanent currency) earned at the end of each wave.',
    max: 10000, gated: true, value: (b) => b, fmt: (b) => '+' + b, gold: curve(50, 1.6), coin: curve(10, 1.0018) },
  { id: 'coinsPerKill', tab: 'economic', icon: 'coinstar', label: 'Coins/Kill',
    name: 'Coins per Kill', tip: 'Coins earned per enemy killed, compounding with kill count.',
    max: 10000, gated: true, value: (b) => b * 0.001, fmt: (b) => '+' + (b * 0.001).toFixed(3),
    gold: curve(60, 1.6), coin: curve(15, 1.0018) },
];
